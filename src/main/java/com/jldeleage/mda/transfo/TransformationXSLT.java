...
//        path += "templates/" + inNomTemplate;
        path += inNomFichierFeuille;
        InputStream input = null;
        if (chargeurRessource != null) {
            input = chargeurRessource.getResource(path);
        }
        else {
            input = new Lecteur().getFichier(path, this);
        }
        setFeuille(input);
//        }
    }
...
//    }
    ...
    public void setParametres(Map<String, String> inParametres) {
        for (String nom : inParametres.keySet()) {
            setParameter(nom, inParametres.get(nom));
        }
    }
