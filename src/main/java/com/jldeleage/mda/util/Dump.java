...
//            Source identite = new StreamSource(Dump.class.getResourceAsStream("/rsrc/xslt/identite.xsl"));
    ...
    public void dump(Document doc) throws ExceptionTransformation {
        if (nomFichierCible == null) {
            dump(doc, System.out);
        }
        else {
            dump(doc, nomFichierCible);
        }
    }
...
//            logger.log(Level.INFO, "dump.dump de " + nomFichier);
            ...
            if (index > 0) {
                String chemin = nomFichier.substring(0, index);
                File dossiers = new File(chemin);
                dossiers.mkdirs();
            }
...
//                        "Transformer du dump OK ");
    ...
    public void setParameter(String nomParametre, String valeur) {
        if ("fichier".equals(nomParametre)) {
            nomFichierCible = valeur;
        }
        super.setParameter(nomParametre, valeur); //To change body of generated methods, choose Tools | Templates.
    }


    @Override
    public void setParametres(Map<String, String> inParametres) {
        nomFichierCible = inParametres.get("fichier");
        super.setParametres(inParametres); //To change body of generated methods, choose Tools | Templates.
    }


    private String    nomFichierCible;
