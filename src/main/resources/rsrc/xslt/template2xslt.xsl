...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
   ...
   <xsl:namespace-alias stylesheet-prefix="xx" result-prefix="xsl"/>
    ...
    <xsl:template match="/">
        <xx:stylesheet version="2.0" exclude-result-prefixes="ete target fn xsi xmi uml_2_3">
            <!-- TODO revoir la nécessité de cette instruction -->
    ...
    <xsl:template match="ete:root">
        ...
        <xx:output encoding="{ $encoding }"
                   name="sortie-template">
            <xsl:apply-templates select="@*" mode="copie-attributs-root"/>
        </xx:output>
...
<!--                            <xsl:message>
            ...
            <xx:result-document format="sortie-template" href="{$href}">
                <xsl:apply-templates>
    ...
    <xsl:comment> - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - </xsl:comment>
        ...
        <xx:function name="ete:snakeToDot">
            <xx:param name="chaine"/>
        ...
        <xx:function name="ete:hasStereotype">
            <xx:param name="noeud"/>
            <xx:param name="stereotype"/>
            <xx:param name="recursif"/>
            <xx:message>
                <xx:text>Recherche du stereotype </xx:text>
                <xx:value-of select="$stereotype"/>
                <xx:text> dans </xx:text>
                <xx:value-of select="$noeud/name"/>
            </xx:message>
            <xx:choose>
                <xx:when test="$noeud/stereotypes/*[name() = $stereotype]">
                    <!--<xx:message>        TROUVE</xx:message>-->
                    <xx:value-of select="true()"/>
                </xx:when>
                <xx:when test="$recursif = true() and $noeud/..">
                    <!--<xx:message>   APPEL RECURSIF</xx:message>-->
                    <xx:value-of select="ete:hasStereotype($noeud/.., $stereotype, true())"/>
                </xx:when>
                <xx:otherwise>
                    <!--<xx:message>      NON TROUVE</xx:message>-->
                    <xx:value-of select="false()"/>
                </xx:otherwise>
            </xx:choose>
        </xx:function>
...
<!--            <xx:message>Appel de la fonction type pour <xx:value-of select="$t"/></xx:message>-->
            <xx:choose>
                ...
                <xx:when test="string-length($t/package)">
        ...
        <!--
            Remplace les caractères non ASCII français par leurs équivalents
            sans marque diacritique.
            Attention, le œ n'est pas géré (car correspond à deux caractères,
            la fonction "translate" ne permet pas de faire cette transformation).
            Exemple d'utilisation :
            €[ete:versAscii(name)]
          -->
        <xx:function name="ete:versAscii">
            <xx:param name="chaine"/>
            <xx:value-of select="translate($chaine, 'àâäéèêëîïôöùûüçñ', 'aaaeeeeiioouuucn')"/>
        </xx:function>
...
<!--            <xx:message>
                <xx:text>Decamelisation de </xx:text>
                <xx:value-of select="$chaine"/>
                <xx:text> -&gt; </xx:text>
                <xx:value-of select="$resultat"/>
            </xx:message>-->
            <xx:value-of select="normalize-space($resultat)"/>
        </xx:function>


        <xx:function name="ete:tronqueUrl">
            <xx:param name="chaine"/>
            <xx:param name="limite"/>
            <xx:message>
                <xx:text>tronqueUrl sur </xx:text>
                <xx:value-of select="$chaine"/>
                <xx:text> après </xx:text>
                <xx:value-of select="$limite"/>
            </xx:message>
            <xx:variable name="resultat">
                    <xx:value-of select="substring-after($chaine,$limite)"/>
            </xx:variable>
            <xx:value-of select="$resultat"/>
            <xx:if test="string-length($resultat) gt 0">
                    <xx:text>/</xx:text>
            </xx:if>
            <xx:message>
                <xx:text>Résultat : </xx:text>
                <xx:value-of select="$resultat"/>
            </xx:message>
        </xx:function>
    ...
    <xsl:template match="@*" mode="copie-attributs-root">
            <xsl:choose>
                <xsl:when test="name(.)='href'">
                    
                </xsl:when>
                <xsl:when test="name(.)='items'">
...
<!--        <xsl:message>Traitement de l'attribut <xsl:value-of select="name()"/></xsl:message>-->
    ...
    <xsl:template match="ete:element">
        <xx:variable name="nom">
            <xsl:apply-templates select="@name" mode="exp-att"/>
        </xx:variable>
        <xx:element>
            <xsl:attribute name="name">
                <xsl:apply-templates select="@name" mode="exp-att"/>
...
<!--        <xsl:message>
        ...
        <xsl:variable name="fichier-importe" select="concat($workingdir, @href)"/>
...
Dossier de travail : <xsl:value-of select="$workingdir"/>
        ...
        <!-- Traduction du template en XSLT. Le résultat est placé dans un
             ...
             Le template peut s'adresser au modèle ou à certains items de ce
             modèle.
             Dans le premier cas, il faut générer une règle "match='/'".
             Dans le deuxième cas, il faut générer une règle "match='nom-item'"
        ...
        <xsl:result-document format="xml" href="{ $href }">
            ...
            <xsl:choose>
                <xsl:when test="document($fichier-importe)/*/@href">
                    <xsl:apply-templates select="document($fichier-importe)"/>
                </xsl:when>
                <xsl:when test="document($fichier-importe)/*/@href2">
...
<!--                        <xx:template match="{ $match-value }">
                            <xsl:if test="document($fichier-importe)/*/@mode">
                                <xsl:attribute name="mode" select="document($fichier-importe)/*"/>
                            </xsl:if>
                            <xsl:if test="document($fichier-importe)/*/@priority">
                                <xsl:attribute name="priority" select="document($fichier-importe)/*/@priority"/>
                            </xsl:if>
                        </xx:template>-->
                        <xx:output name="{ translate(document($fichier-importe)/*/@mode, '=&amp;apos;', '__') }" method="xml"/>
                        <xx:template match="*" mode="{ document($fichier-importe)/*/@mode }">
                            <!-- TODO : INTERPRETER @href -->
                            <xx:result-document format="{ translate(document($fichier-importe)/*/@mode, '=&amp;apos;', '__') }"
                                                href="{ document($fichier-importe)/*/@href}">
                        <xsl:apply-templates select="document($fichier-importe)/*/node()"/>
                            </xx:result-document>
                        </xx:template>
<!--                        <xsl:for-each select="document($fichier-importe)/*/node()">
                            <xx:template match="name">
                        </xsl:for-each>-->
                    </xx:stylesheet>
                </xsl:when>
                <xsl:otherwise>
...
<!--                            <xsl:message>Le document importe n'est pas une feuille "autonome"</xsl:message>
                            <xx:message>Le document importe n'est pas une feuille "autonome"</xx:message>-->
                            <xsl:apply-templates select="document($fichier-importe)/*/node()"/>
                    </xx:template>
                    </xx:stylesheet>
                </xsl:otherwise>
...
<!--        <xsl:message>
    ...
    <!--
      - Utilisée uniquement pour le traitement d'une instruction
      - <ete:element name="xxx"/>
      - Cette instruction doit produire :
      -     
      -->
    <xsl:template match="@*" mode="exp-att">
